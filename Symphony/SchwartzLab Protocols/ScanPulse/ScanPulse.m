classdef ScanPulse < StageProtocol
    
    properties (Constant)
        identifier = 'edu.northwestern.SchwartzLab.ScanPulse'
        version = 1
        displayName = 'Scan Pulse'
    end
    ...
    methods
        ...
        function prepareRun(obj)
         ...
         function prepareEpoch(obj, epoch)            
            ...
            % Add a stimulus to trigger the scanhead
            if ~isempty(obj.rigConfig.deviceWithName('Scanhead_Trigger'))
                %disp('Making scanhead trigger stim');
                p = PulseGenerator();
                
                p.preTime = obj.preTime;
                p.stimTime = 1; %ms
                p.tailTime = obj.stimTime + obj.tailTime - 1;
                p.amplitude = 1;
                p.mean = 0;
                p.sampleRate = obj.sampleRate;
                p.units = Symphony.Core.Measurement.UNITLESS;
                
                epoch.addStimulus('Scanhead_Trigger', p.generate());
            end
