classdef ImageCyclerSwitch < StageProtocol
    
    properties (Constant)
        identifier = 'edu.northwestern.SchwartzLab.ImageCylerSwitch'
        version = 1
        displayName = 'Image Cycler Switch'
    end
    ...
    properties (Constant, Hidden)
       ...
       %imagesDir = 'C:\Users\RigTest\Documents\MATLAB\Symphony\StimulusImages';
    end
        ...
        function p = parameterProperty(obj, parameterName)
            ...
            switch parameterName
                ...
                case 'maskSize'
                    ...
                    p.units = 'um';   
                case {'Nconditions', 'configFile','switchToNegative'}
                    p.displayTab = 'mostUsed';
                case 'meanLevel'
                    p.defaultValue = 0.5;
            end
...
%             %Second image for pre-/tail-time
%             linearInd2 = obj.orderOfImages( mod(obj.numEpochsQueued+1, obj.Nconditions) + 1 );
%             if obj.Nparams == 1
%                 obj.image2name = [obj.config.baseName '_' linearInd2 '.png'];
%                 epoch.addParameter(['im2' obj.imageParams{1}], obj.config.(obj.imageParams{1})(linearInd2)); 
%             elseif obj.Nparams == 2
%                 [b,a] = ind2sub(obj.paramLen,linearInd2);
%                 obj.image2name = [obj.config.baseName '_' num2str(a) '_' num2str(b) '.png'];
%                 epoch.addParameter(['im2' obj.imageParams{1}], obj.config.(obj.imageParams{1})(a)); 
%                 epoch.addParameter(['im2' obj.imageParams{2}], obj.config.(obj.imageParams{2})(b)); 
%                 disp(['Adding ' ['im2' obj.imageParams{1}] ' = ' num2str(obj.config.(obj.imageParams{1})(a))]);
%                 disp(['Adding ' ['im2' obj.imageParams{2}] ' = ' num2str(obj.config.(obj.imageParams{2})(b))]);
%             elseif obj.Nparams == 3
%                 [c,b,a] = ind2sub(obj.paramLen,linearInd2);
%                 obj.image2name = [obj.config.baseName '_' num2str(a) '_' num2str(b) '_' num2str(c) '.png'];
%                 epoch.addParameter(['im2' obj.imageParams{1}], obj.config.(obj.imageParams{1})(a)); 
%                 epoch.addParameter(['im2' obj.imageParams{2}], obj.config.(obj.imageParams{2})(b)); 
%                 epoch.addParameter(['im2' obj.imageParams{3}], obj.config.(obj.imageParams{3})(c)); 
%             else
%                 disp('Error: more than 3 parameters varied in config file');
%             end
            
              %Second image for pre-/tail-time (textures specific, same blur different seed)           
              if obj.Nparams == 2
                  if ~obj.switchToNegative
                      a2 = a;
                      b2set = setdiff((1:obj.paramLen(2)), b);
                      b2 = b2set(randi(obj.paramLen(2)-1));
                      obj.image2name = [obj.config.baseName '_' num2str(a2) '_' num2str(b2) '.png'];
                      epoch.addParameter(['im2' obj.imageParams{1}], obj.config.(obj.imageParams{1})(a2));
                      epoch.addParameter(['im2' obj.imageParams{2}], obj.config.(obj.imageParams{2})(b2));
                      disp(['Adding ' ['im2' obj.imageParams{1}] ' = ' num2str(obj.config.(obj.imageParams{1})(a2))]);
                      disp(['Adding ' ['im2' obj.imageParams{2}] ' = ' num2str(obj.config.(obj.imageParams{2})(b2))]);
                  end;
              end;
        ...
        function preparePresentation(obj, presentation)
            ...
            if obj.switchToNegative
                imOFF = Image(255-curImage);
            else
                curImage2 = imread(fullfile(obj.imagesDir, obj.image2name));
                imOFF = Image(curImage2);
            end
            imOFF.size = obj.windowSize;
            imOFF.position = [obj.windowSize(1)/2, obj.windowSize(2)/2];
            presentation.addStimulus(imOFF);
            imOFF.setMask(mask);
            ...
            function opacity = offDuringStim(state, preTime, stimTime)
                if state.time>preTime*1e-3 && state.time<=(preTime+stimTime)*1e-3
                    opacity = 0;
                else
                    opacity = 1;
            ...
            controller = PropertyController(imOFF, 'opacity', @(s)offDuringStim(s, obj.preTime, obj.stimTime));
            presentation.addController(controller);
