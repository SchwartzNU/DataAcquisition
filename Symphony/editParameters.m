function edited = editParameters(protocol)
    ...
    for paramIndex = 1:paramCount
        ...
        paramCounter(curIndex) = paramCounter(curIndex) + 1;
    end
    
    paramsHeight = max(paramCounter) * 30;
    ...
    % GWS: use tabs
    tabLayout = uiextras.TabPanel( 'Parent', handles.figure );
    mostUsedPanel = uipanel('Parent', tabLayout);
    protocolPanel = uipanel('Parent', tabLayout, ...
        'units', 'normalized', 'position', [0, 0.3, 1, 0.7] );
    projectorPanel = uipanel('Parent', tabLayout);
    ampPanel = uipanel('Parent', tabLayout);
    analysisPanel = uipanel('Parent', tabLayout);
    tabLayout.TabNames = {'MostUsed', 'Protocol', 'Projector', 'Amplifiers', 'Analysis'};
    tabLayout.SelectedChild = 1;
    textFieldParamNames = {};
    panelIndex = zeros(1,length(tabLayout.TabNames));
    tabIndexVec = zeros(1,paramCount);
    for paramIndex = 1:paramCount
        paramName = paramNames{paramIndex};
        ...
        switch paramProps.displayTab
            case 'mostUsed'
                parentTab = mostUsedPanel;  
                curIndex = 1;
            case 'protocol'
                parentTab = protocolPanel;
                curIndex = 2;
            case 'projector'
                parentTab = projectorPanel;
                curIndex = 3;
            case 'amplifier'
                parentTab = ampPanel;
                curIndex = 4;
            case 'analysis'
                parentTab = analysisPanel;
                curIndex = 5;
        end
        tabIndexVec(paramIndex) = curIndex;
        panelIndex(curIndex) = panelIndex(curIndex) + 1;
        ...
        uicontrol(...
            ...
            'Position', [10 dialogHeight-panelIndex(curIndex)*30-15 labelWidth 18], ...
            'String',  paramLabel,...
            'Style', 'text');
        ...
        if isinteger(defaultValue) && ~paramProps.meta.Dependent
            handles.(paramTag) = uicontrol(...
                ...
                'Position', [labelWidth+15 dialogHeight-panelIndex(curIndex)*30-2-15 125 26], ...
            ...
            uicontrol(...
                'Parent', parentTab,...
                'Units', 'points', ...
                'Position', [labelWidth+141 dialogHeight-panelIndex(curIndex)*30+10-15 12 12], ...
                'CData', push_cdata, ...
                'Callback', @(hObject,eventdata)stepValueUp(hObject, eventdata, guidata(hObject), paramTag));
            uicontrol(...
                'Parent', parentTab,...
                'Units', 'points', ...
                'Position', [labelWidth+141 dialogHeight-panelIndex(curIndex)*30-1-15 12 12], ...
        ...
        elseif islogical(defaultValue)
            handles.(paramTag) = uicontrol(...
                ...
                'Units', 'points', ...
                'FontSize', 12,...
                'Position', [labelWidth+15 dialogHeight-panelIndex(curIndex)*30-2-15 140 26], ...
        ...
        elseif isnumeric(defaultValue) || ischar(defaultValue)
            ...
            handles.(paramTag) = uicontrol(...
                ...
                'String',  paramValue,...
                'Style', 'edit', ...
                'Callback', @(hObject,eventdata)valueChanged(hObject, eventdata, guidata(hObject)), ...
                'TooltipString', paramProps.description, ...
                'Tag', paramTag);
        ...
        elseif iscellstr(defaultValue) || (iscell(defaultValue) && all(cellfun(@isnumeric, defaultValue)))            
            ...
            handles.(paramTag) = uicontrol(...
                'Parent', parentTab, ...
                'Units', 'points', ...
                'Position', [labelWidth+15 dialogHeight-panelIndex(curIndex)*30-2-15 140 22], ...
        ...
        if ~isempty(paramProps.units)           
            ...
            handles.(unitsTag) = uicontrol(...
                ...
                'FontSize', 12, ...
                'HorizontalAlignment', 'left', ...
                'Position', [unitsLeft dialogHeight-panelIndex(curIndex)*30-15 60 18], ...
...
%             %paramTag
%             %i
%             %temp = findjobj(handles.(paramTag),'property',{'Tag', paramName});            
%             findjobj(handles.(paramTag));  
%             
%             if length(temp)>1
%                 disp(['param ' paramName ': ' num2str(length(temp)) ' java objects found'])
%                 userData.javaHandle = temp(tabIndexVec(i));
%             else
%                 userData.javaHandle = temp;
%             end
%             set(handles.(paramTag), 'UserData', userData);
%         catch ME %#ok<NASGU>
%             disp(['param ' paramName ': java object not found']); 
%         end
%     end
...
function updateStimuli(handles)
    ...
    if handles.showStimuli
        ...
        elseif any(cellfun(@(s)s.Duration.Equals(TimeSpanOption.Indefinite), stimuli))
            msg = 'Cannot display indefinite stimuli';
        elseif length(unique(cellfun(@(s)char(s.Units), stimuli, 'UniformOutput', false))) > 1
            msg = 'Sample stimuli units are not uniform';
        end
        ...
        if ~isempty(msg)     
            ...
            grid on;
            text('Units', 'normalized', 'Position', [0.5 0.5], 'String', msg, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
        else
            stimulusCount = length(stimuli);
            for i = 1:stimulusCount
                stimulus = stimuli{i};
                block = NET.invokeGenericMethod('System.Linq.Enumerable', 'First', {'Symphony.Core.IOutputData'}, stimulus.DataBlocks(stimulus.Duration.Item2));
                
                data = double(Measurement.ToBaseUnitQuantityArray(block.Data));
                sampleRate = System.Decimal.ToDouble(block.SampleRate.QuantityInBaseUnit);
                units = char(Measurement.HomogenousBaseUnits(block.Data));
                
                plot3(ones(1, length(data)) * i, (1:length(data)) / sampleRate, data);
                hold on
            end
            hold off
