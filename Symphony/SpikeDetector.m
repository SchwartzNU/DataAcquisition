function results = SpikeDetector(D, SampleInterval)
...
%     pause;
   if abs(max(trace)) > abs(min(trace)) %flip it over
    ...
    if ~isempty(peaks) && max(D(i,:)) > min(D(i,:)) %make sure we don't have bad/empty trace
        ...
        [Ind,centroid_amps] = kmeans(peakAmps,3,'start',[median(peakAmps);max(peakAmps)/3;max(peakAmps)],'Options',options);
        ...
        if centroid_amps_sorted(3) - centroid_amps_sorted(2) <  centroid_amps_sorted(2) - centroid_amps_sorted(1) 
