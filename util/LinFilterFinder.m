...
%if not normalizing by stim power spectrum
%FilterFft = mean((fft(response,[],2).*conj(fft(signal,[],2))),1);

FilterFft = (fft(response,[],2).*conj(fft(signal,[],2)))./(fft(signal,[],2).*conj(fft(signal,[],2)));

freqcutoff_adjusted = freqcutoff/(samplerate/length(signal)) ; % this adjusts the freq cutoff for the length
