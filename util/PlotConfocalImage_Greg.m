function PlotConfocalImage_Greg(BaseFileName, MaxProjectionFlag, XYFlag, LocalFlag, verbose, Xrange, Yrange, Zrange)
...
if (LocalFlag)
    RawImageFolderPath = '~/cell_pictures/';
    ImageFolderPath = '~/cell_pictures/processed/';
else
...
if (~exist('ImageParameters') || ~isfield(ImageParameters, 'CompressionExponent'))
    ImageParameters.CompressionExponent = 0.1;     % compression exponent for projected image
end
...
if (~exist('ImageParameters') || ~isfield(ImageParameters, 'MinValue'))
    ImageParameters.MinValue = 0.1;                  % min value for projected image
end
if (~exist('ImageParameters') || ~isfield(ImageParameters, 'ZStackCompressionExponent'))
    ImageParameters.ZStackCompressionExponent = 2;    % compression exponent for zstack
end
if (~exist('ImageParameters') || ~isfield(ImageParameters, 'ZStackMaxValue'))
    ImageParameters.ZStackMaxValue = 2.0;               % max for zstack
end
...
if ~isempty(Zrange)
    ImageParameters.StartFrame = Zrange(1,1);
    ImageParameters.EndFrame = Zrange(1,2);
else
...
if ~isempty(Xrange)
   im = im(Xrange(1,1):Xrange(1,2),:,:); 
end

if ~isempty(Yrange)
   im = im(:,Yrange(1,1):Yrange(1,2),:); 
end
...
while (EndFlag == 0)
    ...
    if (EndFlag == 0 && verbose)
        ...
        cropOK = input('Crop box ok? [0 = no, 1 = yes] ');
        X = [1, size(ProjectedImage,2)];
        Y = [1, size(ProjectedImage,1)];
        while ~cropOK
            [X,Y] = getBox();
            X = round(X)
            Y = round(Y)
        ...
        ProjectedImage = ProjectedImage(Y(1):Y(2),X(1):X(2));
    end    
end
...
if ~isempty(Xrange)
   im2 = im2(Xrange(1,1):Xrange(1,2),:,:); 
end

if ~isempty(Yrange)
   im2 = im2(:,Yrange(1,1):Yrange(1,2),:); 
end
...
if (gain2 > 0)
    ...
    if (XYFlag)
        ...
        if (MaxProjectionFlag)
            for y = 1:size(im2, 1)
                for z = 1:size(im2, 3)
                    ProjectedImage(y, z) = max(im2(:, y, z));
                end
            end
        else
            for r = 1:size(im2, 2);
    ...
    %color to red-green 
    CombinedProjectedImage(:, :, 1) = ScaledProjectedImage2;
    CombinedProjectedImage(:, :, 2) = ScaledProjectedImage;
    CombinedProjectedImage(:, :, 3) = zeros(size(ScaledProjectedImage2));
